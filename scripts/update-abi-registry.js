...
async function fetchAbiVersions() {
  return (
    await getJSONFromCDN('nodejs/node@main/doc/abi_version_registry.json')
...
function electronReleasesToTargets(releases) {
  ...
  return Object.entries(versionsByModules).map(([_major, { version, modules }]) => ({
    abi: modules,
...
async function main() {
  ...
  await writeFile(
    path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'abi_registry.json'),
